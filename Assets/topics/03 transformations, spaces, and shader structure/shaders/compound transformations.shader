    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolators i) : SV_Target
            {
                ...
                // composite = rotate2D * scale2D * translate2D;

                // offset
                float3 uv3 = mul(composite, float3(uv.xy, 0));
                uv = uv3.xy;

                color = rectangle(uv, float2(0.25, 0.5));
                // color += float3(uv.x, 0 , uv.y);

                float rectCenter = 1- step(0.015, length(uv));
                float radius = 0.5;
                float ring = step(radius - 0.005, length(cUV) ) * (1 - step(radius + 0.005, length(cUV)));
                color = (((color + ring) % 2) + rectCenter) % 2;
